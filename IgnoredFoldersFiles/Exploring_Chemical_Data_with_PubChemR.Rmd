...
```{r}
res1 <- get_pug_rest(identifier = "10000", namespace = "cid", domain = "compound", operation = "synonyms")
res2 <- get_synonyms(identifier = "10000", namespace = "cid", domain = "compound")
identical(synonyms(res1), synonyms(res2))
...
# Return synonyms data as a "data.frame"
synonyms(res1)
```

Multiple identifiers is not allowed. In such cases, only the first element in `identifier` will be considered. Please see `get_properties()` for requesting multiple identifiers form PubChem Database. For example:

```{r}
result <- get_pug_rest(identifier = c("1", "2"), namespace = "cid", domain = "compound", 
                       property = c("MolecularFormula","MolecularWeight","CanonicalSMILES"), output = "JSON")

result
```

Above example is identical to the output from `get_properties()` as described below:
...
identical(retrieve(instance(res2)), retrieve(result))
```
