...
## 3. Accessing PUG View with `get_pug_view`
...
### 3.2. Record Summaries and Full Records:
...
```{r, class.output="scroll-100"}
result <- get_pug_view(annotation = "index", identifier = "1234", domain = "compound", output = "JSON")
```
...
### 3.3. Accessing Different Record Types:
...
```{r, class.output="scroll-100"}
result <- get_pug_view(annotation = "image", identifier = "243577", domain = "biologic", output = "SVG")
```
...
```{r, class.output="scroll-100"}
result <- get_pug_view(annotation = "qr", identifier = "1234", domain = "compound", output = "SVG", qrSize = "short")
```
...
### 3.12. Annotation Attachments:

**Specialized Retrieval:** Access attachments like spectral images associated with records:

```{r, class.output="scroll-100"}
result <- get_pug_view(annotation = "data", identifier = "236678_1", domain = "key")
```
