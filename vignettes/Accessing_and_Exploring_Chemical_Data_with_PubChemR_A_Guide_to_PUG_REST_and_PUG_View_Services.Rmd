...
vignette: >
  %\VignetteIndexEntry{Accessing and Exploring Chemical Data with PubChemR: A Guide to PUG REST and PUG View Services}
...
The `PubChemR` package introduces two pivotal functions, `get_pug_rest` and `get_pug_view`, designed to facilitate seamless access to the vast chemical data repository of PubChem. These functions leverage the capabilities of PubChem's Power User Gateway (PUG) REST and PUG View services, providing a straightforward and efficient means for users to programmatically interact with PubChem's extensive database. This vignette aims to elucidate the structure and usage of these services, offering a range of illustrative use cases to aid new users in understanding their operation and constructing effective requests.
...
## 3. PUG View: Detailed Chemical Information and Summaries

PUG View is another REST-style web service that provides detailed information content and summary reports on individual PubChem records, which are not directly available in the primary Substance, Compound, or BioAssay records. It primarily powers the PubChem database summary record web pages but can also be used independently for programmatic access.

### 3.1. Key Features of PUG View:

* **Comprehensive Reports:** Offers complete summary reports on individual PubChem records.
* **Programmatic Access:** Can be used as a standalone service for automated data retrieval and processing.

**Usage Policy:**

* Similar to PUG REST, PUG View is not designed for handling very large volumes of requests. A limit of 5 requests per second is recommended.
* Users with large-scale data needs should contact PubChem for assistance.

## 4. Accessing PUG REST with `get_pug_rest`
...
```{r, class.output="scroll-100", eval=FALSE}
get_pug_rest(identifier = "C1CCCCCC1", namespace = "smiles", domain = "compound",  output = "XML", searchtype = "substructure")
```
